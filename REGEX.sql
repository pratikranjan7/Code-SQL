...
INSERT INTO employees_regex (first_name, last_name, email, job_title, salary, hire_date)
SELECT
    ...
    CASE 
        ...
        ELSE 'HR Manager'
    END AS job_title,
    ROUND(30000 + (random() * 70000)::NUMERIC, 2) AS salary,  -- Casting to NUMERIC
    NOW() - (random() * 1000 * INTERVAL '1 day') AS hire_date
...
INSERT INTO users_regex (email) VALUES
('john.doe@gmail.com'),
...
from users_regex
-- where email ~ '.'
where email ~ '^[a-zA-Z]{3}@gmail.com$'
...
CREATE TABLE media (
    ...
    type VARCHAR(50) NOT NULL  -- 'movie' or 'song'
);
...
SELECT *
FROM employees_regex
where last_name  ~* '[on]' and salary between 50000 and 80000
...
SELECT *
FROM employees_regex
where first_name  ~* '^[aeiou]' and salary between 50000 and 80000
...
SELECT *
FROM employees_regex
where first_name  ~* '^[a-z]{4}$' and last_name  ~* '^[a-z]{5}$'
...
SELECT * FROM feedback
where email ~ '^[A-Za-z]+[-_0-9a-zA-Z\.]*@{1}[a-zA-Z]+\.[a-zA-Z]{2,3}$'

SELECT * FROM feedback
WHERE email ~* '^[a-zA-Z]+(.|_|-|[0-9]*)?[a-zA-Z]{0,}@{1}[a-zA-Z]+\.[a-z]{2,3}$'
...
INSERT INTO users_email (user_id, name, mail) VALUES
